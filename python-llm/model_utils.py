...
def load_model(model_name):
    ...
    try:
        if model_name == "llama2":
            model = AutoModelForCausalLM.from_pretrained('huggingface/llama2', use_auth_token=hf_api_token)
        elif model_name == "mistral":
            model = AutoModelForCausalLM.from_pretrained('huggingface/mistral', use_auth_token=hf_api_token)
        else:
